...
class LocalDateTypeSpec extends FlatSpec {
  ...
  it should "create iterator to other date (inclusive)" in {
    ...
    iter = date ~> (other, Period.ofDays(2))
    assert(iter.next() == date)
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = date ~> (other, Period.ofDays(3))
    assert(iter.next() == date)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other ~> (date, Period.ofDays(-1))
    assert(iter.next() == other)
    assert(iter.next() == LocalDate.parse("2019-07-12"))
    assert(iter.next() == date)
    assert(!iter.hasNext)
    ...
    iter = other ~> (date, Period.ofDays(-2))
    assert(iter.next() == other)
    assert(iter.next() == date)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other ~> (date, Period.ofDays(-3))
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())
  }
