...
class DurationTypeSpec extends FlatSpec {
  ...
  it should "create iterator to other duration (inclusive)" in {
    ...
    var iter = duration ~> (other, Duration.ofMinutes(1))
    ...
    iter = duration ~> (other, Duration.ofMinutes(2))
    assert(iter.next() == duration)
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = duration ~> (other, Duration.ofMinutes(3))
    assert(iter.next() == duration)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other ~> (duration, Duration.ofMinutes(-1))
    assert(iter.next() == other)
    assert(iter.next() == Duration.parse("P33DT5H7M4.123S"))
    assert(iter.next() == duration)
    assert(!iter.hasNext)
    ...
    iter = other ~> (duration, Duration.ofMinutes(-2))
    assert(iter.next() == other)
    assert(iter.next() == duration)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other ~> (duration, Duration.ofMinutes(-3))
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())
  }
