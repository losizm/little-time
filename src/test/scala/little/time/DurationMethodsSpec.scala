...
class DurationMethodsSpec extends org.scalatest.flatspec.AnyFlatSpec:
  ...
  it should "create iterator to other duration (inclusive)" in {
    ...
    var iter = duration.iterateTo(other, Duration.ofMinutes(1))
    ...
    iter = duration.iterateTo(other, Duration.ofMinutes(2))
    assert(iter.next() == duration)
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = duration.iterateTo(other, Duration.ofMinutes(3))
    assert(iter.next() == duration)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other.iterateTo(duration, Duration.ofMinutes(-1))
    assert(iter.next() == other)
    assert(iter.next() == Duration.parse("P33DT5H7M4.123S"))
    assert(iter.next() == duration)
    assert(!iter.hasNext)
    ...
    iter = other.iterateTo(duration, Duration.ofMinutes(-2))
    assert(iter.next() == other)
    assert(iter.next() == duration)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other.iterateTo(duration, Duration.ofMinutes(-3))
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())
  }
  ...
  it should "create iterator to other duration (exclusive)" in {
    ...
    var iter = duration.iterateUntil(other, Duration.ofMinutes(1))
    assert(iter.next() == duration)
    assert(iter.next() == Duration.parse("P33DT5H7M4.123S"))
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = duration.iterateUntil(other, Duration.ofMinutes(2))
    assert(iter.next() == duration)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = duration.iterateUntil(other, Duration.ofMinutes(3))
    assert(iter.next() == duration)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other.iterateUntil(duration, Duration.ofMinutes(-1))
    assert(iter.next() == other)
    assert(iter.next() == Duration.parse("P33DT5H7M4.123S"))
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other.iterateUntil(duration, Duration.ofMinutes(-2))
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other.iterateUntil(duration, Duration.ofMinutes(-3))
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())
  }
