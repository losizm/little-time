...
class LocalTimeTypeSpec extends FlatSpec {
  ...
  it should "create iterator to other time (inclusive)" in {
    ...
    var iter = time ~> (other, Duration.ofMinutes(1))
    ...
    iter = time ~> (other, Duration.ofMinutes(2))
    assert(iter.next() == time)
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = time ~> (other, Duration.ofMinutes(3))
    assert(iter.next() == time)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other ~> (time, Duration.ofMinutes(-1))
    assert(iter.next() == other)
    assert(iter.next() == LocalTime.parse("12:39:45.123456789"))
    assert(iter.next() == time)
    assert(!iter.hasNext)
    ...
    iter = other ~> (time, Duration.ofMinutes(-2))
    assert(iter.next() == other)
    assert(iter.next() == time)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other ~> (time, Duration.ofMinutes(-3))
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())
  }
