...
class YearMonthTypeSpec extends FlatSpec {
  ...
  it should "create iterator to other month (inclusive)" in {
    ...
    iter = month ~> (other, Period.ofMonths(2))
    assert(iter.next() == month)
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = month ~> (other, Period.ofMonths(3))
    assert(iter.next() == month)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other ~> (month, Period.ofMonths(-1))
    assert(iter.next() == other)
    assert(iter.next() == YearMonth.parse("2019-08"))
    assert(iter.next() == month)
    assert(!iter.hasNext)
    ...
    iter = other ~> (month, Period.ofMonths(-2))
    assert(iter.next() == other)
    assert(iter.next() == month)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())

    iter = other ~> (month, Period.ofMonths(-3))
    assert(iter.next() == other)
    assert(!iter.hasNext)
    assertThrows[NoSuchElementException](iter.next())
  }
